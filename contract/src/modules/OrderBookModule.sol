...
abstract contract OrderBookModule is MarginModule {
    ...
    /// @notice 下单（扩展版：支持保证金模式选择）
    /// @param isBuy 是否为买单
    /// @param price 价格
    /// @param amount 数量
    /// @param hintId 插入提示 (可选优化)
    /// @param marginMode 保证金模式（CROSS 或 ISOLATED）
    /// @return 订单 ID
    function placeOrder(bool isBuy, uint256 price, uint256 amount, uint256 hintId, MarginMode marginMode)
        external
        virtual
        nonReentrant
        returns (uint256)
    {
        ...
        if (pos.size == 0) {
            // 新开仓：设置保证金模式
            _setMarginMode(msg.sender, marginMode);
        } else {
            // 加仓/减仓：必须保持相同模式
            require(pos.mode == marginMode, "cannot change margin mode on existing position");
        }
    ...
    returns (uint256 prev, uint256 curr)
{
    ...
    /// @notice 设置保证金模式（仅限新仓位）
    /// @param trader 用户地址
    /// @param mode 保证金模式
    function _setMarginMode(address trader, MarginMode mode) internal {
        Position storage pos = accounts[trader].position;
        pos.mode = mode;
